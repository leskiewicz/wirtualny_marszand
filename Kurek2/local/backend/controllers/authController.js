...
module.exports.register_post = async(req, res) => {
	...
	try {
		...
		if(req.body.password != req.body.repassword) {
			return res.status(404).send({success: false, errors: { passwordsMismatch: "Hasla sie nie zgadzają" }});
		}
		
		const result = await sequelize.transaction(async (t) => {
			
			const user = await User.create({ email: req.body.email, password: req.body.password, accountBalance: 1000.00 }, { transaction: t })
...
module.exports.login_post = async (req, res, next) => {
	try {
		...
		if(!user) {
			res.status(404).send({error: "Nie znaleziono uzytkownika"});
		} else {
			...
			if(userValidate) {
				...
				res.status(404).send({error: "Nieprawidlowe haslo"})
			}
		}
	} catch(err) {
		return res.status(404).send({msg: "Nieprawidlowe parametry"});
	}
}
