...
module.exports.buy_artist_picture_post = async (req, res, next) => {
	...
	try {
		...
		qr.toFile('public/' + qrLocation, fullLink, (err) => {
			if(err) {
				return res.status(404).send({msg: "Nie udalo sie zapisac qrCode"});
			}
		})
...
module.exports.one_picture_get = async (req, res, next) => {
	try {
		const picture = await Picture.findOne({ where: { id: req.params.id, available: true, userId: req.user.id, onSale: false } });
